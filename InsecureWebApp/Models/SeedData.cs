...
namespace MicroFocus.InsecureWebApp.Models
{
    public class SeedData
    {
        ...
        private static void SeedUsers(UserManager<ApplicationUser> userManager)
        {
            ...
            // Ensure demo user exists and force-update 2FA settings.
            var demoEmail = "user@localhost.com";
            var existing = userManager.FindByEmailAsync(demoEmail).Result;
            if (existing == null)
            {
                User user = new User();
                user.UserName = demoEmail;
            ...
            if (existing != null)
            {
                try
                {
                    // Reset the authenticator key so the demo user always has a fresh key
                    // and enable authenticator-based 2FA for demos.
                    userManager.ResetAuthenticatorKeyAsync(existing).Wait();
                    userManager.SetTwoFactorEnabledAsync(existing, true).Wait();
                }
                catch (Exception ex)
                {
                    // Don't break startup on seed errors; the app will still run. If you
                    // want to see seed-time errors, add logging or rethrow.
                    var _ = ex; // swallow intentionally
                }
            }
