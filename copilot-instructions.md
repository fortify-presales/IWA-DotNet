...
Purpose
- This repository is IWA-DotNet (Insecure Web App) — a deliberately insecure .NET 9 application intended for teaching, demos, and security testing (SAST/DAST/IAST/pen-tests).
...
Where to add code
- C# source: `InsecureWebApp/Controllers`, `InsecureWebApp/Pages`, `InsecureWebApp/Models`, `InsecureWebApp/Data` (follow existing project layout in `InsecureWebApp/`).
- Tests: `InsecureWebAppTests/` (add unit or integration tests as appropriate).
- Configuration: `InsecureWebApp/appsettings.json` and `InsecureWebApp/appsettings.Development.json`.
- Sample data and macros: `etc/` and `InsecureWebApp/Files/` if the change requires sample inputs or Postman macros.

Local build & run (quick reference)
- Build the app:

```powershell
dotnet build ./InsecureWebApp/InsecureWebApp.csproj -c Debug
```

- Run locally (Development environment):

```powershell
dotnet run --project ./InsecureWebApp/InsecureWebApp.csproj --environment Development
```

- Run tests:

```powershell
dotnet test ./InsecureWebAppTests/InsecureWebAppTests.csproj
```

- Docker build and run (example):

```powershell
docker build -t iwa -f InsecureWebApp/Dockerfile InsecureWebApp
docker run --rm -p 5000:80 -e ASPNETCORE_ENVIRONMENT=Development iwa
```
