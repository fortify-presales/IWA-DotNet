...
namespace MicroFocus.InsecureWebApp.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<Microsoft.AspNetCore.Identity.SignInResult> AuthUser([FromForm] string email, [FromForm] string pwd, [FromForm] bool RememberMe)
        {
            ...
            // Log detailed sign-in result flags to help debug 2FA / lockout behavior for demo users.
            _logger.LogInformation("SignInResult for {User}: Succeeded={Succeeded}, RequiresTwoFactor={RequiresTwoFactor}, IsLockedOut={IsLockedOut}, IsNotAllowed={IsNotAllowed}",
                sUserName, result.Succeeded, result.RequiresTwoFactor, result.IsLockedOut, result.IsNotAllowed);
            ...
            {
                _logger.LogInformation("User {0} requires two-factor authentication. Redirect to 2FA flow.", sUserName);
            }
